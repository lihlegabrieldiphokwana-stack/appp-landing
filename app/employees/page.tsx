...
const employeeBenefits = [
  {
    ...
    icon: (
      <svg className="w-6 h-6 text-b-ink" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const modeSwitcherCards = [
  {
    ...
    icon: (
      <svg className="w-8 h-8 text-b-ink-soft" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    icon: (
      <svg className="w-8 h-8 text-b-ink" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function EmployeesPage() {
  return (
    <main className="min-h-screen bg-b-paper">
      ...
      <section className="pt-32 pb-20 px-6 border-b border-b-line">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="text-xs font-semibold tracking-widest text-b-ink/70 uppercase mb-4">
              FOR EMPLOYEES
            </div>
            <h1 className="text-5xl md:text-7xl font-display font-extrabold text-b-ink tracking-tight mb-6">
              Your workday,<br />
              <span className="text-b-green-deep">organized.</span>
            </h1>
            <p className="text-b-ink-soft text-lg md:text-xl leading-relaxed mb-8 max-w-xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link
                href="/download"
                className="px-8 py-3.5 bg-b-ink hover:bg-b-forest text-b-paper font-semibold rounded-full transition-colors"
              >
                Download Bouul
              </Link>
              <Link
                href="#how-it-works"
                className="px-8 py-3.5 border border-b-ink/20 text-b-ink-soft hover:text-b-ink hover:border-b-ink/50 font-medium rounded-full transition-colors"
              >
                How it works →
              </Link>
            </div>
      ...
      {/* Key benefits */}
      <section className="py-24 bg-b-paper border-b border-b-line">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {employeeBenefits.map((benefit, i) => (
              <motion.div
                ...
                className="rounded-2xl border border-b-line bg-b-paper-raised p-6 flex gap-4"
              >
                <div className="w-12 h-12 rounded-xl bg-b-green-soft border border-b-green/30 flex items-center justify-center flex-shrink-0">
                  {benefit.icon}
                </div>
                <div>
                  <h3 className="text-b-ink font-semibold mb-1.5">{benefit.title}</h3>
                  <p className="text-b-ink-soft text-sm leading-relaxed">{benefit.description}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      {/* How it works */}
      <section id="how-it-works" className="py-24 bg-b-paper border-b border-b-line">
        <div className="max-w-7xl mx-auto px-6">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {howItWorksSteps.map((step, i) => (
              ...
              >
                <div className="w-10 h-10 rounded-full bg-b-ink/10 border border-b-ink/15 flex items-center justify-center text-b-ink font-bold text-sm mb-4">
                  {step.step}
                </div>
                <h3 className="text-b-ink font-semibold text-lg mb-2">{step.title}</h3>
            ...
            {modeSwitcherCards.map((card, i) => (
              <motion.div
                ...
                className={`rounded-2xl border p-8 ${
                  card.active
                    ? "border-b-green/30 bg-b-green-soft"
                    : "border-b-line bg-b-paper-raised"
                }`}
              >
                <div className="mb-4">{card.icon}</div>
                <h3 className={`text-xl font-semibold mb-2 ${card.active ? "text-b-ink" : "text-b-ink-soft"}`}>
                  {card.title}
                </h3>
                <p className="text-b-ink-soft text-sm leading-relaxed">{card.description}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 bg-b-paper border-b border-b-line">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={point} className="flex items-start gap-3">
                    <div className="mt-1.5 h-1.5 w-1.5 rounded-full bg-b-ink flex-shrink-0" />
            ...
            >
              <div className="space-y-4">
                <div className="pb-4 border-b border-b-line">
                  <div className="text-b-ink text-sm font-semibold">Team Chat</div>
                  <div className="text-b-ink-soft text-xs">All messages tied to bookings</div>
                </div>
                <div className="space-y-3">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-3 text-sm text-b-ink-soft">
                      <div className="w-1.5 h-1.5 rounded-full bg-b-green flex-shrink-0" />
                      {item}
                    </div>
                  ))}
                </div>
      ...
      <section className="py-24 bg-b-paper">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <motion.div
            ...
            transition={{ duration: 0.6 }}
          >
            <h2 className="text-4xl md:text-6xl font-display font-extrabold text-b-ink mb-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link
                href="/download"
                className="px-10 py-5 bg-b-ink hover:bg-b-forest text-b-paper font-semibold rounded-full text-lg transition-colors"
              >
                Download Bouul
              </Link>
              <Link
                href="/vendors"
                className="px-10 py-5 border border-b-ink/20 text-b-ink-soft hover:text-b-ink hover:border-b-ink/50 font-medium rounded-full text-lg transition-colors"
              >
                For business owners →
              </Link>
            </div>
