        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-6 mb-6">
            <motion.article
              ...
              className="rounded-3xl border border-b-line bg-b-paper-raised p-8 md:p-10"
            >
              <div className="text-xs font-semibold tracking-widest text-b-green-deep uppercase mb-4">
                Featured story
              </div>
              <h2 className="text-3xl md:text-5xl font-display font-extrabold text-b-ink tracking-tight mb-5">
                {featuredArticle.title}
              </h2>
              <p className="text-b-ink-soft text-lg leading-relaxed mb-6">
                {featuredArticle.summary}
              </p>
              <div className="flex flex-wrap gap-4 text-xs uppercase tracking-widest text-b-ink-soft mb-8">
            ...
            <motion.aside
              ...
              className="rounded-3xl border border-b-line bg-b-paper p-8"
            >
              <div className="text-xs font-semibold tracking-widest text-b-ink-soft uppercase mb-5">
                What we cover
              </div>
              <div className="space-y-4">
                {[
                  ...
                  <div key={item} className="flex items-start gap-3">
                    ...
                    <p className="text-b-ink-soft text-sm leading-relaxed">{item}</p>
                  </div>
                ))}
              </div>
            </motion.aside>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {latestArticles.map((story, index) => (
              ...
              >
                ...
                <h2 className="text-2xl font-semibold text-b-ink mb-4">
                  {story.title}
                </h2>
          ...
          <div className="rounded-3xl border border-b-line bg-b-paper-raised p-8">
            ...
            <div className="space-y-5">
              {newsroomUpdates.map((item) => (
                <div key={item.title} className="border-b border-b-line pb-4 last:border-0 last:pb-0">
                  <h3 className="text-b-ink font-semibold mb-2">{item.title}</h3>
            ...
            <div className="space-y-4">
              {newsroomPressAssets.map((asset) => (
                <a
                  ...
                  href={asset.href}
                  download
                  className="flex items-center justify-between gap-4 rounded-2xl border border-b-line bg-b-paper-raised px-4 py-4 hover:border-b-ink/40 transition-colors"
                >
                  <div>
                    <div className="text-b-ink font-semibold">{asset.label}</div>
                    <div className="text-b-ink-soft text-sm">{asset.note}</div>
                  </div>
                  <div className="text-b-green-deep text-sm font-medium">Download</div>
                </a>
              ))}
            </div>
            <div className="mt-8 rounded-2xl border border-dashed border-b-ink/20 bg-b-paper-raised/70 p-5">
              <div className="text-b-ink font-semibold mb-2">Media contact</div>
              <p className="text-b-ink-soft text-sm leading-relaxed">
