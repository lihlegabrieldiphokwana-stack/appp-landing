      ...
      <div className="relative h-full p-4 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          {roles.map((role, index) => (
            <div key={role} className="rounded-2xl border border-b-ink/10 bg-b-ink/5 p-3">
        ...
        <div className="grid grid-cols-3 gap-2 flex-1 min-h-0">
          {["Move fast", "Keep trust", "Make it simple"].map((value) => (
            <div key={value} className="rounded-[1.6rem] border border-b-ink/10 bg-b-paper/30 p-3 text-[11px] text-b-ink-soft">
              {value}
            </div>
          ))}
        </div>
