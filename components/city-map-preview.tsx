...
const cityPins = [
  { name: "Johannesburg", x: "52%", y: "42%", tone: "bg-b-green" },
  ...
  return (
    <div className="absolute inset-0 bg-b-paper-raised text-b-ink">
      ...
      <div className="relative h-full p-4 flex flex-col gap-4">
        ...
        <div className="rounded-[1.6rem] border border-b-ink/10 bg-b-ink/5 p-4 backdrop-blur-2xl flex-1 min-h-0">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="rounded-full border border-b-ink/10 bg-b-paper/35 px-3 py-1 text-[10px] text-b-ink-soft">
              12 cities
            </div>
          </div>

          <div className="relative h-[320px] rounded-[1.4rem] border border-b-ink/10 bg-[linear-gradient(rgba(255,255,255,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.03)_1px,transparent_1px)] bg-[size:28px_28px] overflow-hidden">
            ...
            {cityPins.map((pin) => (
              ...
              >
                ...
                <div className="mt-2 rounded-full border border-b-ink/10 bg-b-paper/80 px-2 py-1 text-[9px] text-b-ink/85 whitespace-nowrap">
                  {pin.name}
                </div>
              </div>
            ))}
        ...
        <div className="grid grid-cols-3 gap-2">
          {cityCards.map((card) => (
            <div key={card.city} className="rounded-2xl border border-b-ink/10 bg-b-ink/5 p-3">
              <div className="text-[10px] uppercase tracking-[0.2em] text-b-ink-soft mb-1">
                {card.city}
              </div>
              <div className="text-sm font-semibold">{card.status}</div>
              <div className="text-[11px] text-b-green-deep mt-1">{card.demand}</div>
            </div>
          ))}
        </div>
      </div>
