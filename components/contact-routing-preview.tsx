...
const routes = [
  { title: "Support", detail: "Bookings, accounts, and app help", tone: "bg-b-green" },
    ...
    <div className="absolute inset-0 bg-b-paper-raised text-b-ink">
      ...
      <div className="relative h-full p-4 flex flex-col gap-4">
        ...
        <div className="space-y-2">
          {routes.map((route) => (
            <div key={route.title} className="rounded-2xl border border-b-ink/10 bg-b-ink/5 px-3 py-3 flex items-center justify-between">
              <div>
                <div className="text-sm font-medium">{route.title}</div>
                <div className="text-[11px] text-b-ink-soft">{route.detail}</div>
              </div>
        ...
        <div className="grid grid-cols-3 gap-2 flex-1 min-h-0">
          <div className="rounded-[1.6rem] border border-b-ink/10 bg-b-ink/5 p-3">
            <div className="text-[9px] uppercase tracking-[0.2em] text-b-ink-soft mb-1">Inbox</div>
            <div className="text-sm font-semibold">0 waiting</div>
          </div>
          <div className="rounded-[1.6rem] border border-b-ink/10 bg-b-ink/5 p-3">
            <div className="text-[9px] uppercase tracking-[0.2em] text-b-ink-soft mb-1">Live chat</div>
            <div className="text-sm font-semibold">Open</div>
          </div>
          <div className="rounded-[1.6rem] border border-b-ink/10 bg-b-ink/5 p-3">
            <div className="text-[9px] uppercase tracking-[0.2em] text-b-ink-soft mb-1">Email</div>
