  ...
  return (
    <div className="absolute inset-0 bg-b-paper-raised text-b-ink">
      ...
      <div className="relative h-full p-4 flex flex-col gap-4">
        ...
        <div className="relative flex-1 rounded-[1.6rem] border border-b-ink/10 bg-b-paper/30 p-4 overflow-hidden">
          ...
          <div className="relative grid grid-cols-2 gap-3 h-full">
            {pages.map(([title, detail], index) => (
              <div
                key={title}
                className={`rounded-2xl border border-b-ink/10 bg-b-ink/5 p-3 ${
        ...
        <div className="grid grid-cols-3 gap-2">
          {["Product", "Trust", "Launch"].map((item) => (
            <div key={item} className="rounded-2xl border border-b-ink/10 bg-b-ink/5 px-3 py-2 text-[11px] text-b-ink-soft">
              {item}
            </div>
          ))}
        </div>
