  ...
  return (
    <div className="absolute inset-0 bg-b-paper-raised text-b-ink">
      ...
      <div className="relative h-full p-4 flex flex-col gap-4">
        ...
        <div className="relative flex-1 rounded-[1.6rem] border border-b-ink/10 bg-b-ink/5 p-4 overflow-hidden">
          ...
          <div className="grid grid-cols-2 gap-3 h-full">
            {nodes.map((node, index) => (
              <div key={node.label} className={`flex items-center justify-center ${index === 1 || index === 3 ? "pt-10" : "pb-10"}`}>
                <div className={`rounded-full ${node.tone} px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.22em] text-b-forest shadow-lg shadow-black/30`}>
                  {node.label}
                </div>
              </div>
            ))}
