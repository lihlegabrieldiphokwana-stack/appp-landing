      ...
      <div className="relative h-full p-4 flex flex-col gap-4">
        ...
        <div className="rounded-[1.6rem] border border-b-ink/10 bg-b-ink/5 p-4 backdrop-blur-2xl">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="rounded-2xl border border-b-ink/10 bg-b-paper/35 px-3 py-2 text-right">
              <div className="text-[9px] uppercase tracking-[0.2em] text-b-ink-soft">Launch</div>
              <div className="text-sm font-semibold text-b-green-deep">Ready</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {assets.map((asset, index) => (
            <div key={asset.label} className="rounded-2xl border border-b-ink/10 bg-b-ink/5 p-3">
              <div className="h-24 rounded-xl border border-b-ink/10 bg-gradient-to-br from-white/10 to-black/20 p-2">
                {asset.shape === "mark" && (
                  <div className="flex h-full items-center justify-center">
                    <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-b-green text-lg font-black text-b-forest">
                      B
                    </div>
                  </div>
                )}
                {asset.shape === "wide" && (
                  <div className="h-full rounded-lg bg-b-paper/35 p-2">
                    <div className="h-3 w-20 rounded-full bg-b-green/80" />
                    <div className="mt-5 h-3 w-28 rounded-full bg-white/20" />
                    <div className="mt-2 h-2 w-16 rounded-full bg-b-ink/10" />
                  </div>
                )}
                {asset.shape === "phone" && (
                  <div className="mx-auto h-full w-12 rounded-xl border border-b-ink/15 bg-b-paper/45 p-1.5">
                    <div className="h-3 w-5 rounded-full bg-white/15 mx-auto mb-2" />
                    <div className="h-12 rounded-lg bg-b-green/20" />
                  </div>
                )}
                {asset.shape === "screens" && (
                  <div className="relative h-full">
                    <div className="absolute left-4 top-3 h-14 w-16 rounded-lg border border-b-ink/15 bg-b-paper/45" />
                    <div className="absolute right-4 bottom-2 h-16 w-10 rounded-xl border border-b-ink/15 bg-b-green/15" />
                  </div>
                )}
              </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <div key={label} className="rounded-2xl border border-b-ink/10 bg-b-paper/30 p-3">
              ...
              <div className="mt-1 text-[11px] text-b-ink-soft">{detail}</div>
            </div>
          ))}
        </div>
      </div>
