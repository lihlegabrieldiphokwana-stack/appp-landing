      ...
      <div className="relative h-full p-4 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-2 flex-1 min-h-0">
          {scenarios.map((scenario, index) => (
            <div key={scenario.title} className={`rounded-[1.4rem] border border-b-ink/10 bg-gradient-to-br ${scenario.tone} p-3`}>
              <div className="flex items-start justify-between gap-2">
                <div className="rounded-full bg-b-paper/35 px-2 py-1 text-[9px] uppercase tracking-[0.2em] text-b-ink/90">
                  {index + 1}
                </div>
                <div className="h-2.5 w-2.5 rounded-full bg-b-green" />
              </div>
              <div className="mt-4 text-sm font-semibold leading-tight">{scenario.title}</div>
              <div className="mt-3 h-16 rounded-xl border border-b-ink/10 bg-b-paper/25" />
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {["Discovery", "Tracking", "Growth"].map((label) => (
            <div key={label} className="rounded-2xl border border-b-ink/10 bg-b-ink/5 px-3 py-2 text-[11px] text-b-ink-soft">
              {label}
            </div>
          ))}
        </div>
